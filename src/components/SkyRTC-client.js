const SkyRTC = function () {
    ...
    skyrtc.prototype.connect = function (server) {
        ...
        this.on('__peers', function (data) {
            console.info('__peers', data)
            //获取所有服务器上的
        ...
        this.on('__invite', function(data) {
            console.info('__invite')
        })

        this.on('__reject', function(data) {
            console.info('__reject')
        })

        this.on('__ring', function(data) {
            console.info('__ring')
        })

        this.on('__cancel', function(data) {
            console.info('__cancel')
        })

        this.on('__join', function(data) {
            console.info('__join')
        })

        this.on("__ice_candidate", function (data) {
            console.info('__ice_candidate', data)
            var candidate = new nativeRTCIceCandidate({
                sdpMLineIndex: data.label,
                sdpMid: data.id,
                candidate: data.candidate
            });
            var pc = that.peerConnections[data.fromID];
        ...
        this.on('__new_peer', function (data) {
            console.info('__new_peer', data)
            that.connections.push(data.userID);
            var pc = that.createPeerConnection(data.userID)
            pc.addStream(that.localMediaStream);
            that.emit('new_peer', data.userID);
        });

        this.on('__remove_peer', function (data) {
            console.info('__remove_peer')
            var sendId;
        ...
        this.on('__offer', function (data) {
            console.info('__offer')
            that.receiveOffer(data);
            that.emit("get_offer", data);
        });

        this.on('__answer', function (data) {
            console.info('__answer', data)
            that.receiveAnswer(data);
    ...
    function createStreamSuccess(stream) {
        console.info('createStreamSuccess', stream)
        if (gThat) {
    ...
    skyrtc.prototype.createRoom = function (userId) {
        console.info('createRoom', userId)
    ...
    skyrtc.prototype.sendInvite = function(from, to) {
        console.info('sendInvite')
        this.from = from
        this.socket.send(JSON.stringify({
            "eventName": "__invite",
            "data": {
    ...
    skyrtc.prototype.receiveOffer = function (data) {
        this.sendAnswer(data);
    };

    //发送answer类型信令
    skyrtc.prototype.sendAnswer = function (data) {
        console.info('sendAnswer', data)
        var pc = this.peerConnections[data.fromID];
        var that = this;
        //pc.setRemoteDescription(new nativeRTCSessionDescription(sdp));
        pc.setRemoteDescription(new nativeRTCSessionDescription({
            'type': 'answer',
            'sdp': data.sdp
        }));
    ...
    skyrtc.prototype.receiveAnswer = function (data) {
        console.info('receiveAnswer', data)
        var pc = this.peerConnections[data.fromID];
        pc.setRemoteDescription(new nativeRTCSessionDescription({
            'type': 'answer',
            'sdp': data.sdp
    ...
    //创建单个PeerConnection
    skyrtc.prototype.createPeerConnection = function (to) {
        console.info('createPeerConnection')
        var that = this;
        var pc = new PeerConnection(iceServer);
        this.peerConnections[to] = pc;
        pc.onicecandidate = function (evt) {
            console.info('onicecandidate', evt.candidate)
            if (evt.candidate)
                that.socket.send(JSON.stringify({
            ...
            that.emit("pc_get_ice_candidate", evt.candidate, to, pc);
        };

        pc.onopen = function () {
            that.emit("pc_opened", to, pc);
        };

        pc.onaddstream = function (evt) {
            that.emit('pc_add_stream', evt.stream, to, pc);
        };

        pc.ondatachannel = function (evt) {
            that.addDataChannel(to, evt.channel);
            that.emit('pc_add_data_channel', evt.channel, to, pc);
        };
        return pc;
    };
